...
Two thresholds, two behaviors: below the buffered CCTP fee a deposit parks at detection (bridging is *impossible*, not merely uneconomical); above it the deposit always bridges — converging funds on Arbitrum, the one chain all recovery lives on — but a bridged amount outside the Boltz pair limits parks *minted*, before any lock exists: moving invoice resolution ahead of the lock carried the limit check with it, so the reference's lock + cooperative-refund round-trip (two sponsored txs for nothing) never happens; cooperative refunds remain for genuinely post-lock failures. Refund returns are detected as ordinary Arbitrum inflows (sender = the swap contract) and park as `RefundReturned` — never auto-retried, since a failed flow re-entering by itself would loop. Recovery (`retry_parked`) aggregates every Arbitrum-side parked balance into ONE lock unit (deposit↔lock is N:1 exactly here; sub-limit refunds can never individually clear the pair minimum) and is integrator-triggered, following the `accept_degraded_quote` precedent of user-gating retries of things that already failed once.
