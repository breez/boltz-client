...
## 2026-06-09 — Gate sponsored-claim confirmation on the EIP-5792 top-level status, not the per-receipt status
Diverges from boltz-web-app: yes — the web app has the same Alchemy EIP-7702 gas-sponsor flow (`src/alchemy/Alchemy.ts`, consistent with the 2026-05-29 entry), but its `waitForTransactionHash` confirms a sponsored call on the mere *presence* of a receipt (`receipts[0].transactionHash`), gating on **neither** the top-level EIP-5792 numeric `status` nor the per-receipt `status` hex — so it never detects a reverted claim at this layer (its only `status === "reverted"` check is on a separate *bridge-send* tx, `SwapExecutionWorker.tsx:839`). Our original per-receipt `0x1`/`0x0` gate was already stricter; this change refines it to the authoritative top-level code.

The claim is submitted through Alchemy's gas sponsor as an ERC-4337 UserOperation wrapped by an EIP-5792 `wallet_sendPreparedCalls`, and we confirm it by polling `wallet_getCallsStatus`. The subtlety — easy to get wrong, and we initially did — is that **a successful on-chain receipt does not mean the claim succeeded.** The ERC-4337 `EntryPoint` wraps each UserOp's execution in a try/catch: when the inner call reverts it catches the revert, charges gas, and lets the outer bundler `handleOps` transaction succeed with receipt status `0x1`, recording the failure only in the `UserOperationEvent.success` flag and a `UserOperationRevertReason` log. So the per-receipt `status` hex that `wallet_getCallsStatus` returns — which mirrors the raw `handleOps` tx receipt — stays `0x1` even for a reverted claim. The original `poll_call_status` gated success/revert solely on that per-receipt `0x1`/`0x0`, so a reverted sponsored claim could be mistaken for success: the claim hash would be persisted and the swap advanced (a bridged swap would then sit in `Settling` forever waiting on a delivery that never happened, since the claim never ran).

The authoritative revert signal is the **EIP-5792 top-level numeric `status` code**: `200–299` = "included onchain without reverts", `≥300` (esp. `500`/`600`) = reverted/failed, `100–199` = pending. This is what viem's decoder keys off and what Alchemy's own aa-sdk consumes, so the sponsor must return it. Verified against open source: rundler's `eth_getUserOperationReceipt` carries `success: bool` (from `UserOperationEvent`) *separate* from the `handleOps` `TransactionReceipt`; aa-sdk re-exports `getCallsStatus` straight from viem, which classifies by the numeric ranges above. `classify_calls_status` therefore uses the top-level code only as a **revert override** — a code `≥300` is a hard failure, checked *first* so a reverted claim can never be confirmed via its `0x1` receipt — and otherwise confirms on the per-receipt status exactly as the original client did. This is deliberately a *monotonic* change over the original per-receipt gate: identical on success, strictly better on reverts. Gating *success* on the top-level code was rejected because we have not yet seen a real Alchemy response (the boltz-web-app checkout types the field as `status: number` but never reads its value), so assuming success == `200–299` could hang a real claim if the value differed; the revert-override design can't (a real revert we fail to classify just degrades to the original behaviour). Confirmed against a live mainnet claim (2026-06-09): the `wallet_getCallsStatus` result carries a top-level numeric `status` (observed `100` while pending — no `receipts` yet — then `200` on success, alongside `receipts[0].status: "0x1"`), so the field the revert override keys on is present and a real revert (`≥300`) would be caught. (This also disproved a prior client assumption that the field was a flat `1`/`0` — it is the HTTP-style EIP-5792 code.)
