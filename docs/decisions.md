...
Completion gated on `forwardTxHash`, set only when Circle's forwarding service performs the mint. Circle's forward-to-Solana is best-effort: a first-time recipient's forward creates the recipient's USDC ATA and mints in one shot, which can fail (`forwardState: FAILED` / `INTERNAL_ERROR`) and then park forever with no auto-retry. When the mint lands some other way (a third-party `receiveMessage`), Iris keeps reporting `FAILED` / null `forwardTxHash`, so the swap never left `Settling` — a liveness defect, not fund loss (the amount is fixed in the attested message, recoverable indefinitely). Fix: on a `FAILED` forward, confirm from the destination chain — the Solana `used_nonce` PDA (`[b"used_nonce", nonce]` under the destination `MessageTransmitterV2`) exists iff `receiveMessage` ran, a sweep-proof, forwarder-agnostic proof of the mint (recipient balance is not — funds are swept in seconds). The nonce is read from the attested `message` (header offset 12), not a separate Iris field. `forwardState` (a vendor field) is only the *trigger* — completion still rests on the on-chain nonce — and gating on it keeps this fallback off the happy path: a healthy in-progress forward triggers no Solana RPC. The bet is that `FAILED` is the only forward state needing the fallback; if an undocumented stall state ever appears, swaps park in `Settling` (funds safe, ops can act) rather than complete wrongly. The crate stays a read-only observer (no self-relay); a `FAILED` forward is logged but never marked `Failed`. Solana-only: EVM mints create no ATA so don't hit this failure mode, and an EVM `usedNonces` probe is left as a drop-in seam behind `cctp_destination_delivered` rather than pre-paying an ever-growing per-chain-RPC cost for a risk that isn't occurring.
