...
|---|---|---|
| `platform-utils` (`http/`, `proxy.rs`, `auth.rs`, `lib.rs`) | reqwest-based `HttpClient` (one backend for native and WASM; native adds HTTP/2 + TCP keepalives, WASM omits the User-Agent), unified `HttpError` with `.status()`, the `ProxyConfig` every transport routes through, and target-correct `tokio`/`time` re-exports. Client construction is fallible: a proxy that cannot be applied errors rather than falling back to a direct connection. WASM gate is `all(target_family="wasm", target_os="unknown")`. | `HttpClient`, `HttpError`, `HttpResponse`, `DefaultHttpClient`, `create_http_client`, `create_http_client_with_proxy`, `ProxyConfig`, `ContentType` |
