...
| `evm/provider.rs` | Thin WASM-safe JSON-RPC client for read-only Arbitrum ops (`eth_call`, `eth_getTransactionReceipt`, `eth_chainId`, `eth_getLogs`, `eth_blockNumber`, `eth_l1_block_number`) with 429 backoff. `eth_call` is pinned to `latest`. `eth_block_number` returns the **L2** height; `eth_l1_block_number` reads the Arbitrum `l1BlockNumber` field for the **L1** height that Solidity `block.number` / a swap's `timeout_block_height` are denominated in. | `EvmProvider`, `TxReceipt` (`is_success`), `LogEntry` |
